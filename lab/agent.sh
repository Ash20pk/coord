...
# anything, and knoot's turn-start context is invisible to it by construction.
...
# where knoot hands the agent its peers, what moved under it, and its mail.
...
run_agent() {
  ...
  first=$(KNOOT_USER="$name" claude -p "$task" \
  ...
  for (( t = 2; t <= TURNS; t++ )); do
    echo "--- turn $t ---"
    KNOOT_USER="$name" claude -p --resume "$sid" \
