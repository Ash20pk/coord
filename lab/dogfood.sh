...
REPO_URL="${KNOOT_DOGFOOD_URL:-https://github.com/expressjs/express.git}"
WORK="${KNOOT_DOGFOOD_DIR:-$HOME/knoot-dogfood}"
...
OUT="${KNOOT_LAB_OUT:-/tmp/knoot-dogfood}"
...
case "${1:-run}" in
report) knoot_metrics "$DB" "$(cat "$OUT/.repo" 2>/dev/null || true)"; exit 0 ;;
clean)
...
[[ -f "$REPO/.knoot.toml" ]] || (cd "$REPO" && "$COORD" init --relay "ws://${RELAY_ADDR}/ws" >/dev/null)
# `knoot init` writes hook config into the checkout; keep it out of the diff.
grep -qx '.knoot.toml' "$REPO/.git/info/exclude" 2>/dev/null || \
...
Run \`knoot who\` before you edit to see who holds what. If a write is refused,
you will be told who holds the file and why — wait, pick a different part of
your task, or use \`knoot msg <user|all> "text"\` to negotiate. Say when you are
